...
#' @name intensity_calc
#' @import ggplot2
#' @importFrom ggplot2 ggplot aes geom_line geom_point scale_color_manual labs theme_minimal theme facet_wrap
#' @keywords internal
...
intensity_calc <- function(intensity, production, use_hdata = TRUE) {
  ...
  if (length(intensity) <= 49) {
    ...
    ggplot2::ggplot(df, ggplot2::aes(x = Step)) +
      ggplot2::geom_point(ggplot2::aes(y = Intensity, color = "Light Intensity"), size = 3) +
      ggplot2::geom_line(ggplot2::aes(y = Intensity, color = "Light Intensity"), size = 1.5) +
      ggplot2::geom_point(ggplot2::aes(y = Production, color = "Production"), size = 3, shape = 17) +  # Triangle for production
      ggplot2::geom_line(ggplot2::aes(y = Production, color = "Production"), size = 1.5, linetype = "dashed") +
      ggplot2::scale_color_manual(values = c("forestgreen", "darkorange")) +
      ggplot2::labs(title = "Visualization of Light Intensity and Production (Up to 49 Steps)",
           x = "Steps",
           y = "Values",
           color = "Legend") +
      ggplot2::theme_minimal(base_size = 15) +
      ggplot2::theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 18),
            legend.position = "top",
    ...
    ggplot2::ggplot(df, aes(x = Step)) +
      ggplot2::geom_point(aes(y = Intensity, color = "Light Intensity"), size = 3) +
      ggplot2::geom_line(aes(y = Intensity, color = "Light Intensity"), size = 1.5) +
      ggplot2::geom_point(aes(y = Production, color = "Production"), size = 3, shape = 17) +
      ggplot2::geom_line(aes(y = Production, color = "Production"), size = 1.5, linetype = "dashed") +
      ggplot2::scale_color_manual(values = c("forestgreen", "darkorange")) +
      ggplot2::labs(title = "Visualization of Light Intensity and Production (More than 49 Steps)",
           x = "Steps",
           y = "Values",
           color = "Legend") +
